...
then
  ...
  if [[ "$TRAVIS_BRANCH" == "staging" ]]; then
    ...
    export REACT_APP_SCORES_SERVICE_URL="http://test-driven-staging-alb-656138088.us-east-1.elb.amazonaws.com"
  elif [[ "$TRAVIS_BRANCH" == "production" ]]; then
    export DOCKER_ENV=prod
    export REACT_APP_USERS_SERVICE_URL="http://testdriven-production-alb-1067420407.us-east-1.elb.amazonaws.com"
    export REACT_APP_EXERCISES_SERVICE_URL="http://testdriven-production-alb-1067420407.us-east-1.elb.amazonaws.com"
    export REACT_APP_SCORES_SERVICE_URL="http://testdriven-production-alb-1067420407.us-east-1.elb.amazonaws.com"
  ...
  then
    ...
    # client
    docker build $CLIENT_REPO -t $CLIENT:$COMMIT -f Dockerfile-$DOCKER_ENV --build-arg REACT_APP_USERS_SERVICE_URL=$REACT_APP_USERS_SERVICE_URL --build-arg REACT_APP_API_GATEWAY_URL=$REACT_APP_API_GATEWAY_URL
    ...
    # exercises
    docker build $EXERCISES_REPO -t $EXERCISES:$COMMIT -f Dockerfile-$DOCKER_ENV --build-arg REACT_APP_USERS_SERVICE_URL=$REACT_APP_USERS_SERVICE_URL
